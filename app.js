...
function paceForecast() {
  ...
  /* Во сколько внутренних единиц обходится одна показываемая. У книги и курса
     это одно и то же, у пьесы такт стоит полутора десятков проходов. */
  const perUnit = unit === "bar" && piece().bars ? total / piece().bars : 1;
  if (!left) return { left: 0, rate: 0, days: 0, unit, perUnit, done: true };
  ...
  return { left, rate: темп, days: Math.max(1, Math.ceil(left / темп)), unit, perUnit, done: false };
}

/* ── Скорость словами ──
   Показываем скорость в день, а не «сколько за вечер». Это то самое число,
   из которого получается дата: остаток делить на него. «Девятнадцать страниц
   за вечер» рядом с «к началу октября» спорило бы само с собой — по такому
   счёту выходит семнадцать дней, а не двадцать девять. Разница между двумя
   числами — ровно те дни, когда книга лежала закрытой.

   Медленному материалу день не годится: «ноль страниц в день» — не ответ.
   Тогда меряем неделей. */
const UNIT_WORD = {
  page: ["стр.", "стр.", "стр."],
...
function paceSpeed(f) {
  ...
  /* Внутри пьеса считается проходами — так срок точнее: такт, набравший
     ...
     из внутренней кухни, по нему не понять, движется разбор или нет.
     Показываем то, что человек и правда закрывает: такты. Пересчёт линейный,
     поэтому скорость и срок по-прежнему об одном и том же. */
...
function paceHTML() {
  ...
  /* «7 дн. из 12»: сколько дней занимался и сколько прошло с первого захода.
     ...
     считаются занятием. */
  const занятий = curStats().days || 0;
  const первый = entries().map((e) => e.date).sort()[0];
  const всего = первый ? Math.max(занятий, daysBetween(первый, todayStr()) + 1) : 0;
  return `<span class="pace">${subLine(
    занятий ? `${занятий} дн. из ${всего}` : "",
    paceSpeed(f),
    paceText(w)
  )}</span>`;
}
...
function gmSpravki(p) {
  ...
  const имя = p.q || [p.name, вид === "place" ? "" : (p.t || "").split(" · ")[0]]
    .filter(Boolean).join(" ");
  const вопрос =
    вид === "book" ? `Расскажи коротко про книгу ${имя}.${книга} Ответь по пунктам, каждый в одно-два предложения: 1) о чём она; 2) чем известна и почему её помнят; 3) кому её стоит читать, а кому нет; 4) есть ли русский перевод и какой считается лучшим. Без пересказа сюжета и без спойлеров.`
  : вид === "person" ? `Расскажи коротко про ${имя}.${книга} Ответь по пунктам, каждый в одно-два предложения: 1) кто это и когда жил; 2) чем известен; 3) какая одна история про него стоит того, чтобы её знать; 4) с чего начать знакомство — что прочитать, посмотреть или послушать. Покажи фотографии.`
  : вид === "animal" ? `Расскажи коротко про ${имя}.${книга} Ответь по пунктам, каждый в одно-два предложения: 1) кто это и как выглядит; 2) где живёт и что делает; 3) чем интересна — одна особенность, которую не ожидаешь; 4) где в природе её можно увидеть. Покажи фотографии.`
  : вид === "art" ? `Расскажи коротко про ${имя}.${книга} Ответь по пунктам, каждый в одно-два предложения: 1) кто и когда это сделал; 2) что на этой работе происходит и на что смотреть; 3) чем она известна; 4) где её можно увидеть сегодня. Покажи изображения в хорошем качестве.`
  : вид === "thing" ? `Расскажи про ${имя}.${книга} Ответь по пунктам, каждый в одно-два предложения: 1) что это за вещь и как она устроена; 2) как её делали и из чего; 3) какое место она занимала в быту; 4) сохранились ли такие вещи и где на них посмотреть. Покажи изображения находок.`
  : вид === "rock" ? `Расскажи про породу ${имя}.${книга} Ответь по пунктам, каждый в одно-два предложения: 1) как она образуется; 2) как выглядит и чем отличается на вид от соседних; 3) где её больше всего; 4) какие известные скалы и здания из неё сложены. Покажи фотографии образцов и скал.`
  : `Что такое «${имя}»?${книга} Ответь по пунктам, каждый в одно-два предложения: 1) что это простыми словами; 2) откуда взялось слово; 3) как это выглядит и работает на деле; 4) с чем это чаще всего путают. Покажи фотографии или схемы.`;
  ...
  return `<span class="gm-links">
    <a href="https://www.google.com/search?tbm=isch&q=${фото}" target="_blank" rel="noopener">Фото</a>
    <a href="https://www.perplexity.ai/search?q=${q}"${мимо}>Подробнее</a>
    <a href="https://chatgpt.com/?q=${q}"${мимо}>ChatGPT</a>
    <a href="https://ya.ru/search/?text=${q}"${мимо}>Яндекс</a>
  </span>`;
}
...
function gmCard() {
  ...
  const откуда = кн
    ...
    : "";
  /* Четыре вопроса вместо общего «расскажи»: имя, правда, настоящее время и —
     главное — что уцелело настоящего. Открывая карту, хочется знать не историю
     вообще, а можно ли встать там, где стоял герой, и что он оттуда видел.
     Длину задаём жёстко: без ограничения ответ разрастается на две страницы. */
  const вопрос = encodeURIComponent(
    `Расскажи коротко про место ${p.q || p.name}.${откуда} `
    + `Ответь по четырём пунктам, на каждый одно-два предложения: `
    + `1) откуда взялось название; `
    + `2) что здесь происходило и насколько это подтверждено — где легенда, а где раскопки и документы; `
    + `3) что на этом месте сейчас; `
    + `4) что сохранилось подлинного, той самой эпохи, и куда идти, чтобы это увидеть. `
    + `Пиши коротко и конкретно, без общих фраз и перечней дат. `
    + `Сюжет книги не пересказывай. Покажи фотографии — современные и старые.`);
  const история = `https://www.perplexity.ai/search?q=${вопрос}`;
