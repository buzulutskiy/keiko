...
function pianoWeekPlan() {
  ...
  /* Сыгранное сверх дневной нормы в неделю не идёт. Раньше остаток считался от
     всех минут подряд, и заход про запас съедал следующие дни: просидел без
     пианино до субботы, увидел «75 минут», отыграл два часа — и в воскресенье
     вместо тех же семидесяти пяти просили тридцать. Выходило, что за лишний
     час работы тебе сокращают завтрашний, то есть усердие само себя и
     отменяет. Теперь каждый прошедший день отдаёт в неделю не больше своей
     нормы: лишнее остаётся просто сыгранным.
     Норму прошедшего дня пересчитываем тем же правилом, что и сегодняшнюю, —
     по остатку на тот момент. Хранить её не надо: при простое ряд выходит
     22 · 25 · 30 · 38 · 50 · 75 · 150, и он однозначно восстанавливается из
     минут по дням. */
  let зачтено = 0;
  for (let d = 0; d < деньНедели; d++) {
    const норма = Math.ceil(Math.max(0, цель - зачтено) / (7 - d));
    зачтено += Math.min(поДням[d], норма);
  }
  const впереди = 7 - деньНедели;                          // считая сегодняшний
  const остаток = Math.max(0, цель - зачтено);
  const надо = остаток ? Math.ceil(остаток / впереди) : 0;
  /* Счётчика два, и они про разное.
     «Зачтено» решает, сколько просить сегодня, и лишнего не берёт — иначе
     субботний запас крал бы воскресенье.
     «Набрано» решает, молчать ли вовсе, и смотрит на все отыгранные минуты
     подряд. Если мерить его тоже зачётом, выходит наоборот нелепость: отыграл
     всю недельную цель в понедельник — и до воскресенья тебя всё равно
     просят, к субботе уже о двух часах, потому что зачёт взял с понедельника
     только его двадцать две минуты. Человек отыграл два с половиной часа, а
     приложение считает, что он отстал. Так что лишнее не переносится вперёд,
     но и не пропадает: неделю оно закрывает. */
  const неделя = поДням.reduce((a, b) => a + b, 0);
  const набрано = неделя >= цель;
  return { цель, неделя, сегодня, зачтено,
