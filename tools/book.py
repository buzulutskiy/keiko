...
Сети здесь только структурные — те, что не судят: топонимы находятся по
словам-указателям («улица», «мост», «двор»), сноски издания составитель уже
отметил сам. Словарь эпохи из scan убран: он был один на все книги, для Гоголя
пересказывал очевидное, а для Данте вернул бы ноль. Сеть, которая делает вид,
что выбирает, хуже её отсутствия — на неё полагаешься.

Выбирают чтением. Двенадцать проходов регулярками по «Петербургским повестям»
стоили 84 тысячи токенов и упустили половину топонимов; прочитать все пять
повестей целиком — 114 тысяч. Для этого есть `text`.
...
Спека — один json. Обязательное:
    ...
    ach[], words{}, themes[], map[], maps[]

Карт бывает несколько — тогда в спеке maps[{key, name, зум, поле}], а у точки
поле m с ключом её карты (пусто — первая). Первая пишется без суффикса, чтобы
её видели старые версии приложения.
...
def глава_текстом(путь, номер):
    """Одну главу целиком, чтобы её прочитать.

       Двенадцать проходов регулярками по «Петербургским повестям» стоили
       восемьдесят четыре тысячи токенов и всё равно упустили половину
       топонимов: сеть находит то, что в неё заложено, а не то, что в книге.
       Прочитать все пять повестей целиком — сто четырнадцать тысяч. Разница
       в тридцать тысяч, а результат несравним."""
    текст = open(путь, encoding="utf-8").read() if путь.endswith(".md") else конвертировать(путь)
    куски = re.split(r'^## ', текст, flags=re.M)[1:]
    свои = [k for k in куски if not re.match(r'^(Перед текстом|Оглавление|Над книгой|Сноски)',
                                             k.split("\n")[0].strip())]
    if not (1 <= номер <= len(свои)): sys.exit(f"глав {len(свои)}, а просят {номер}")
    print("## " + свои[номер - 1].rstrip())


def конвертировать(epub):
    md = subprocess.run([sys.executable, EPUB2MD, epub], capture_output=True, text=True)
    for s2 in (md.stdout + md.stderr).splitlines():
        if s2.strip().endswith(".md"): return open(s2.strip(), encoding="utf-8").read()
    sys.exit("epub не сконвертировался:\n" + md.stderr[:400])
...
def проверить_награды(spec):
    ...
    if spec.get("mode") == "parts":
        ...
        for i in range(1, len(точки)):
            # Шестьдесят страниц — это примерно три вечера чтения. Мерка от
            # читателя, а не от книги: сорок было бы придиркой к толстому тому,
            # а сто — пропуском провала в тонком.
            if точки[i] - точки[i - 1] > 60: провалы.append((точки[i - 1], точки[i]))
    print(f"\n── награды ──")
...
def проверить_собрание(spec):
    ...
    # Спутать легко: у сборника с титулом и содержанием тянет написать сюда
    # номер первой страницы текста — и книга сразу показывает процент, хотя
    # её не открывали.
    ст = spec.get("startPage", 0)
    первая = (spec["chapters"] or [{}])[0].get("from", 0)
    if ст and abs(ст - (первая - 1)) <= 1:
        print(f"  ⚠ startPage {ст} совпадает с началом текста ({первая}) — "
              f"похоже, поле понято как «где начинается текст». Оно значит "
              f"«докуда уже прочитано»: книга покажет {ст / spec['pages'] * 100:.0f}% "
              f"до первой отметки.")
    страниц = max(1, spec["pages"] - ст)
    средняя = len(вещи) / страниц          # плотность самой книги, от неё и пляшем
    print(f"\n── собрание ──")
    ...
    for i, c in enumerate(гл):
        ...
        # Ругаемся не на число и не на порог, а на отклонение от самой книги.
        # «Глава длиннее пятнадцати страниц с двумя записями» — правило для
        # прозы с бытом эпохи; у нон-фикшна такая плотность нормальна, и
        # проверка кричала одиннадцать раз подряд там, где всё в порядке.
        # Короткие главы не считаем: предисловие, заключение и благодарности
        # бывают на две страницы, и пустыми им быть положено.
        знак = " ⚠" if стр >= 8 and (n / стр > 3 * средняя or n / стр < средняя / 3) else ""
...
def build(путь_спеки):
    ...
    # хранения на устройстве. С постоянной единицей телефон держал первую
    # подложку вечно, а координаты приезжали новые — и метки «съезжали».
    статья = {"map": карта, "mapVer": int(time.time()), "savedAt": int(time.time() * 1000)}
    # Карт у книги может быть несколько: у «Мёртвых душ» пол-империи и семь
    # адресов на Невском — в одном масштабе это не живёт. Каждая описана в
    # спеке своим ключом, зумом и именем; у точки поле `m` говорит, чья она.
    # Первая карта пишется без суффикса — так её видят старые версии.
    наборы = spec.get("maps") or []
    if места and наборы:
        первый = наборы[0].get("key", "")
        for p in места: p["m"] = p.get("m") or первый
        for точка, готовая in zip([x for x in spec.get("map", []) if not x.get("kind")], места):
            точка["m"] = готовая["m"]
        вых = []
        for i, м in enumerate(наборы):
            k = м.get("key", "")
            свои = [p for p in места if p.get("m", "") == k]
            if not свои: sys.exit(f"у карты «{м.get('name', k)}» нет ни одной точки")
            рамка, картинка = подложка(свои, поле=м.get("поле", 0.012),
                                       зум=м.get("зум", 14))
            имя = f"art-map-{key}.txt" if i == 0 else f"art-map-{key}-{k}.txt"
            файлы_каталога[имя] = ("data:image/jpeg;base64," +
                base64.b64encode(open(картинка, "rb").read()).decode())
            вых.append({"key": k, "name": м.get("name", k), "box": рамка})
            print(f"  карта «{м.get('name', k)}»: точек {len(свои)}")
        статья["maps"] = вых
        статья["mapBox"] = вых[0]["box"]      # старые версии читают только его
    elif места:
...
if __name__ == "__main__":
    if len(sys.argv) < 3 or sys.argv[1] not in ("scan", "build", "check", "text"):
        sys.exit(__doc__)
    if sys.argv[1] == "scan":
        разведка(sys.argv[2], sys.argv[3] if len(sys.argv) > 3 else None)
    elif sys.argv[1] == "text":
        глава_текстом(sys.argv[2], int(sys.argv[3]))
