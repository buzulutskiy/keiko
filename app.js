...
function gmCard() {
  ...
  /* Прямой вызов Яндекс Браузера его собственной схемой: адрес кладётся в неё
     закодированным в base64. Схема нигде официально не описана, поэтому это
     проба: если приложения нет, iOS покажет свою ошибку — отдельной кнопкой,
     чтобы это не мешало обычной ссылке рядом.
     Адрес уже percent-encoded, то есть чистый ASCII, и btoa на нём безопасен;
     на всякий случай всё равно в try. */
  let ябро = "";
  try { ябро = "yandexbrowser-open-url://" + btoa(алиса); } catch {}
