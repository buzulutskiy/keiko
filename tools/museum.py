...
Формат входного файла:
    ...
    поля: глава | предмет | инв. № | музей | зал или город | статус | почему это здесь | ссылка | запрос | картинка | значок | вид | справка | шаг
...
def разобрать(текст):
    ...
    for сырая in текст.splitlines():
        ...
        справка = поля[12] if len(поля) > 12 else ""  # готовый рассказ о самом предмете
        # у курса вместо главы бывает шаг: предмет открывается ровно там, где велено
        # взять его в руки, а не на первом же закрытом шаге всего урока
        шаг = поля[13] if len(поля) > 13 else ""
        ...
        out.append({
            ...
            **({"about": справка} if справка else {}),
            **({"step": int(шаг)} if шаг.strip().isdigit() else {}),
        })
    return out
