...
function gmCard() {
  ...
  /* И вторая беда — длина. Наш подробный вопрос после кодирования разрастался
     до трёх тысяч знаков, а всё это встаёт в адресе на место имени узла, где
     столько не помещается: приложение открывалось, а адрес до него не доезжал.
     Поэтому Яндекс Браузеру отдаём короткий запрос — место, книга, «кратко»;
     остальные кнопки по-прежнему несут полный вопрос. */
  let ябро = "";
  try {
    const коротко = `${p.q || p.name}${кн ? ` (${кн.title})` : ""} — история места кратко`;
    const адрес = `https://ya.ru/search/?text=${encodeURIComponent(коротко)}`;
    const b64 = btoa(адрес).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
    ябро = "yandexbrowser-open-url://" + b64;
  } catch {}
