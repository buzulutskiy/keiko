...
/* ── Карта открывается там, где читаешь ── */
{
  ...
  /* Пустая глава всё равно подставляется: карта — справочник по книге, и
     «в этой главе ничего нет» честнее, чем свалка всех точек разом. */
  t.get("data").book.entries.push({ id: "2", date: "2026-08-02", bookId: "kniga", page: 45 });
  ок("карта: пустая глава тоже подставляется", t.get("mapHereChapter")(), 3);
...
/* ── Слои карты ── */
{
  ...
  ок("слои: перечислены те, что есть", t.get("gmLayersOf")().map((x) => x[0]), ["place", "book", "person"]);
  t.get("gm").места.push({ name: "Олуша", kind: "animal", lat: 0, lon: 0 },
                         { name: "Дюльфер", kind: "word", lat: 0, lon: 0 });
  ок("слои: живность и слова тоже видны",
    t.get("gmLayersOf")().map((x) => x[0]), ["place", "book", "person", "animal", "word"]);
  ...
  t.get("gm").слой = "book";
  ок("слои: переключились на книги", t.get("gmВидимые")().map((p) => p.name), ["Книга"]);
  ок("содержание: глава считается по всем слоям сразу", t.get("gmParts")().map((c) => c.k), [1]);
