...
/* ── Текст: предложение как единица нажатия ──
   Пальцем айфон выделяет по словам, и собрать вопрос из пары фраз получалось
   долго. Нажатие берёт предложение целиком, соседнее — растягивает. */
{
  const режь = t.get("gmSentences");
  ок("предложения: обычный абзац",
    режь("Он вышел из дому. Пошёл дождь! Куда теперь?").length, 3);
  ок("предложения: сокращение не режет",
    режь("Стало быть, т. е. проще говоря, он ушёл. И всё.").length, 2);
  ок("предложения: инициалы не режут",
    режь("Пришёл И. И. Иванов. Сел.").length, 2);
  ок("предложения: многоточие — конец",
    режь("Он молчал… Потом ушёл.").length, 2);
  ок("предложения: тире реплики — конец",
    режь("Он замолчал. — Ну и ладно, — сказал я.").length, 2);
  ок("предложения: пробел остаётся при фразе",
    режь("Раз. Два.")[0], "Раз. ");
  ок("предложения: абзац без точки — целиком",
    режь("Без точки совсем").length, 1);

  const было = { gm: t.get("gm"), фразы: t.get("gmФразы"), поиск: t.get("document.querySelector") };
  t.set("document.querySelector", () => null);   // подсветке не за что цепляться
  t.set("gmФразы", ["Первое. ", "Второе. ", "Третье."]);
  t.set("gm", { места: [], id: "kn", слой: "read", часть: 1, at: null,
    чтение: 0, чтениеГл: 1, метка: null, выд: null, поиск: "", части: [] });
  const тап = t.get("gmTapSentence");
  тап(1);
  ок("нажатие: одно предложение", t.get("gmВыдТекст")(), "Второе.");
  тап(2);
  ок("нажатие: соседнее растягивает", t.get("gmВыдТекст")(), "Второе. Третье.");
  тап(0);
  ок("нажатие: и назад тоже", t.get("gmВыдТекст")(), "Первое. Второе. Третье.");
  t.set("gm.выд", { a: 1, b: 1 });
  тап(1);
  ок("нажатие: по себе же — снимает", t.get("gm").выд, null);
  t.set("document.querySelector", было.поиск);
  t.set("gmФразы", было.фразы); t.set("gm", было.gm);
}
   ...
   Показывался только первый раздел части, и до остальных было не добраться. */
{
  ...
  ок("метка: одна глава — без счётчика", t.get("gmМетка")(разделы, 2, 3, 2), "");
  ок("метка: внутри части — где ты", t.get("gmМетка")(разделы, 0, 4, 2), "3 из 4");
