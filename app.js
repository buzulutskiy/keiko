...
function paceHTML() {
  ...
  /* Дневной минимум «попробуй дочитать до 238-й страницы» убран: следить за
     нормой на день оказалось незачем, и подсказка только перекрывала собой
     единственное, что тут правда полезно — когда это кончится при нынешнем
     темпе. Срок пересчитывается сам, по последним занятиям. */
  const w = paceWhen(f);
