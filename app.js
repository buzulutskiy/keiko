...
function предметHTML(x, список) {
  ...
  const вопрос = encodeURIComponent(
    вид === "живое"
      ? `Расскажи коротко про ${имяQ}. Не больше пяти-шести предложений: как выглядит, `
        + `где водится или растёт, чем живёт, когда и где это можно увидеть своими глазами. `
        + `Только конкретное, без общих фраз. Покажи фотографии.`
      : вид === "порода"
