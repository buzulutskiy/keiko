...
/* ── Траектория: отметить произведения одним подтверждением ── */
{
  const было = t.get("data");
  ...
  const note = sandbox.recordRouteSession(b, {
    chapters: [{ items: [
      { key: "Стихотворения 1815|А", name: "А" },
      { key: "Стихотворения 1815|Б", name: "Б" },
    ] }],
  }, new Set(["Стихотворения 1815|А", "Стихотворения 1815|Б"]), "2026-09-26");
  ок("траектория: создаёт запись в заметках", d.thoughts.length, 1);
  ок("траектория: заметка привязана к книге", note.key, b.id);
  ок("траектория: заметка перечисляет прочитанное", note.text, "По траектории прочитал: А; Б");
  sandbox.recordRouteSession(b, { chapters: [{ items: [
    { key: "Стихотворения 1815|А", name: "А" },
    { key: "Стихотворения 1815|Б", name: "Б" },
  ] }] }, new Set(["Стихотворения 1815|Б", "Стихотворения 1815|А"]), "2026-09-26");
  ок("траектория: одинаковая заметка не дублируется", d.thoughts.length, 1);
  const routeChecks = t.get("routeHasChecks");
  ок("траектория: галочки есть у вкладки стихов", routeChecks({ name: "Стихи", chapters: [] }), true);
  ок("траектория: галочки есть у вкладки статей", routeChecks({ name: "Статьи", chapters: [] }), true);
  ок("траектория: пустой маршрут без галочек", routeChecks({ name: "Пусто" }), false);
  t.set("data", было);
}
