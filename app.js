...
function gmNavRow() {
  ...
  /* Выбор карты показываем, только когда он настоящий: в этой главе точки
     правда лежат на разных картах. У «Мёртвых душ» так вышло у одной главы из
  ...
  const разные = new Set(gmМестаГлавы().map(картаТочки)).size > 1;
  gmMapRow(карты, своя && разные);
