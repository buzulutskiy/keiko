...
/* ── Как часто садишься ──
   Считаем по тому же окну, что и срок. Правило несимметричное, и это
   нарочно: что материал стоит — видно и по одному заходу, если он был месяц
   назад. А «трижды в неделю» из одного захода не следует: вчера порисовал,
   и только. */
...
/* Как идёт в последнее время — словами, а не долей. Доля от календаря умеет
   только падать: пропущенную неделю не отыграть, и дальше процент тянет вниз,
   как ни занимайся. Слова ходят в обе стороны — вернулся после месяца, и
   через неделю тут снова «через день».

   Формулировки нарочно описательные: что происходит, а не хорошо ли это.
   «Почти каждый день» — не похвала, «раз в неделю» — не упрёк. */
const OFTEN_WORD = [
  [5,   "почти каждый день"],
  [3,   "через день"],
...
function paceOftenText(list) {
  ...
  /* Простой считаем от последнего захода, а не по среднему за окно. Окно из
     пяти заходов размазывает тишину: пять вечеров августа, растянутые на
     месяц, дают «раз в неделю», хотя книгу не открывали тридцать дней.
     Неделя без единого захода — уже простой, и по одному заходу это видно
     тоже. */
  if (daysBetween(дни[дни.length - 1], todayStr()) > 7) return IDLE_WORD();
  // а вот частота из одного захода не следует: вчера сел — и только
...
function paceDaysHTML(list) {
  const дни = paceDays();
  if (!дни) return "";
  return `<span class="pace">${subLine(дни, paceOftenText(list))}</span>`;
}
