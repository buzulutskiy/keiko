...
function gmSpravki(p) {
  ...
  : вид === "text" ? `${имя}.${книга} Расскажи об этом коротко и по существу: 1) как это устроено на самом деле; 2) откуда это известно — кто и как выяснил; 3) в чём с этим до сих пор спорят. Пиши конкретно, без общих фраз. Сюжет книги не пересказывай.`
...
function gmCard() {
  ...
  /* Главный вопрос к месту — не «что тут было», а «что от этого осталось и
     ...
     заканчиваются списком точек, куда ехать.
     Фотографии просим отдельной строкой и обязательными: без этого ответ
     приходит сплошным текстом, а место запоминается видом, а не датами.
     Длину задаём жёстко — иначе разрастается на две страницы. */
  const вопрос = encodeURIComponent(
    ...
    + `3) что от той эпохи уцелело на самом месте — какие именно развалины, стены, храмы, и в каком музее лежат находки отсюда; `
    + `4) что здесь стоит увидеть сегодня: три-четыре главные достопримечательности по названиям, чем каждая интересна и как до неё добираются; `
    + `5) одна живая подробность, которую замечают те, кто там побывал. `
    + `Обязательно покажи фотографии: общий вид места сегодня, снимки того подлинного, что сохранилось, и старые фотографии, если они есть. `
    + `Пиши конкретно, без общих фраз и перечней дат. Сюжет книги не пересказывай.`);
  const история = `https://chatgpt.com/?q=${вопрос}`;
  const мимо = (navigator.standalone === true) ? "" : ` target="_blank" rel="noopener"`;
  card.hidden = false;
