...
function gmCard() {
  ...
  /* Название ищем НЕ по всему миру, а в окрестностях самой точки: адрес вида
     /maps/search/<имя>/@широта,долгота,16z открывает карту на нужном месте и
     уже там ищет имя. Не нашлось — просто останешься на этом месте, а не
     уедешь к однофамильцу на другой континент. Где координата поставлена на
     глазок, имя не передаём вовсе. */
  const наглаз = /стоит примерно/.test(p.t || "");
  const гео = (p.q && !наглаз)
    ? `https://www.google.com/maps/search/${encodeURIComponent(p.q)}/@${p.lat},${p.lon},16z`
