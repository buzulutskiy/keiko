   ...
   обещает «завтра закончишь». */
{
  ...
  /* Скорость в строке — та же, из которой получается дата: остаток, делённый
     на неё, должен давать те же дни. Иначе строка спорит сама с собой. */
  книга(200, [["2026-03-01", 10], ["2026-03-02", 20], ["2026-03-03", 30],
              ["2026-03-04", 40], ["2026-03-05", 50]]);
  t.set("todayStr", () => "2026-03-05");
  const ф = t.get("paceForecast")();
  ок("скорость: 10 стр. в день", t.get("paceSpeed")(ф), "10 стр. в день");
  ок("скорость: сходится с датой", Math.ceil(ф.left / ф.rate), t.get("paceWhen")(ф).days);

  /* Пьеса внутри считается проходами — так срок точнее. Но показывать надо
     то, что человек закрывает: такты. Проход стоит 1/16 такта. */
  ок("скорость: пьеса меряется тактами, а не проходами",
    t.get("paceSpeed")({ unit: "bar", rate: 7.33, perUnit: 15.75 }), "3 такта в неделю");
  ок("скорость: без пересчёта была бы кухня",
    t.get("paceSpeed")({ unit: "bar", rate: 7.33, perUnit: 1 }), "7 тактов в день");

  /* Медленное чтение: «ноль страниц в день» — не ответ, меряем неделей. */
  книга(400, [["2026-03-01", 2], ["2026-03-08", 4], ["2026-03-15", 6]]);
  t.set("todayStr", () => "2026-03-15");
  ок("скорость: медленному материалу — неделя",
    t.get("paceSpeed")(t.get("paceForecast")()), "3 стр. в неделю");
