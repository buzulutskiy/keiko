...
/* ── Теория пьесы приходит после занятия, а не из справочника ──
   Заходить читать теорию просто так незачем: мотивация есть ровно в минуту,
   когда закрыл сессию. Те же заметки, что лежали в карте, выдаются по одной. */
{
  ...
  t.set("data", { active: "piano",
    ...
    pastel: { courses: [], entries: [] }, watch: { videos: [], entries: [] }, practice: {} });
  t.set("CATALOG", { bwv: { ach: [] } });
  t.set("ARTS", { bwv: { map: [
    { part: 1, kind: "word", name: "Октава", t: "почему это та же нота", about: "Частота вдвое больше." },
    { part: 1, kind: "word", name: "Полутон", t: "самый короткий шаг", about: "Соседние клавиши." },
  ...
  ок("теория: первая — из начала файла", шаг(), "Октава");
  ок("теория: дальше по порядку", шаг(), "Полутон");
  ок("теория: и следующий блок", шаг(), "Каденция");
  ок("теория: кончились — не выдумываем", t.get("theoryNext")(), null);
  ок("теория: прочитанное запомнено", t.get("theorySeen")().length, 3);

  /* Собрание у пьесы — те же заметки: очередь выдала после занятия, значит она
     же открылась в собрании. Один механизм на две поверхности, и потому им
     нечем разойтись. */
  {
    const было = t.get("MUSEUM");
    t.set("MUSEUM", { items: [
      { id: "m1", book: "bwv", name: "Октава", kind: "теория" },
      { id: "m2", book: "bwv", name: "Не выдана", kind: "теория" },
    ] });
    ок("собрание: выданная открыта", t.get("musOpen")({ book: "bwv", name: "Октава" }), true);
    ок("собрание: невыданная закрыта", t.get("musOpen")({ book: "bwv", name: "Не выдана" }), false);
    ок("собрание: чужая пьеса не наша",
      t.get("musOpen")({ book: "нет-такой", name: "Октава" }), false);
    t.set("MUSEUM", было);
  }

  /* Заметки без текста в очередь не идут: показывать заголовок не за чем. */
  t.set("ARTS", { bwv: { map: [{ part: 1, name: "Пустая" }] } });
  t.get("data").practice.bwv.theory = [];
  ок("теория: без справки запись пропускается", t.get("theoryNext")(), null);
