   ...
   втрое, так что от неё нет толку. */
{
  ...
  /* Только текущая глава: ни вперёд, ни назад. Прошлое засоряет короткий
     список, будущее спойлерит. */
  ок("подсказки: из прошлых глав не берём",
    t.get("askSuggest")("подж", 6).length, 0);
  ок("подсказки: из непрочитанного тоже",
  ...
  /* Слова живут одну главу: перешёл дальше, не разобрав, — прошлые уходят.
     Тащить их за собой значит копить долг, а долг превращает всё в домашку. */
  t.get("data").book.entries.push({ id: "e2", date: "2026-09-02", bookId: "kn", page: 250 });
  ок("слова: в новой главе прошлых нет", t.get("asksOpen")().length, 0);
  t.get("asksSweep")();
  ок("слова: и убраны они пометкой, а не вырезаны", t.get("asksAll")().length, 2);
  ок("слова: чистка помечает удалёнными",
    t.get("asksAll")().filter((a) => a.deleted).length, 2);
