...
function gmCard() {
  ...
  /* Прямой вызов Яндекс Браузера его собственной схемой. Формат подбирали по
     тому, что приложение делало с нашей ссылкой.
     Сначала клали адрес в base64 — приложение открывалось, но страница нет.
     Улика: браузер показывал «http://ahr0chm6ly…», то есть брал всё после
     «://» и открывал это как адрес, подставив http:// спереди. А раз это уже
     имя домена, система привела его к нижнему регистру — и base64, который
     регистрозависим, разрушился.
     Значит, схема ждёт обычный адрес как есть, без всякого кодирования.
     Запрос держим коротким: он едет внутри чужого адреса, и чем он короче,
     тем меньше поводов его обрезать. */
  let ябро = "";
  try {
    const коротко = `${p.q || p.name}${кн ? ` (${кн.title})` : ""} — история места кратко`;
    ябро = `yandexbrowser-open-url://https://ya.ru/search/?text=${encodeURIComponent(коротко)}`;
  } catch {}
  ...
  card.innerHTML = `
    ...
    <div class="gm-links">
      ...
      ${ябро ? `<a href="${esc(ябро)}">Я.Браузер</a>` : ""}
