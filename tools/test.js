   ...
   обещает «завтра закончишь». */
{
  ...
  ок("строка: но про простой говорим прямо", /не читаю/.test(t.get("paceHTML")()), true);
...
/* ── Простой рисунок: день плюс снимок ── */
{
  ...
  /* Частота словами, а не долей: слова ходят в обе стороны, доля — только
     вниз. Формулировки описательные, без похвалы и упрёка. */
  const былСегодня = t.get("todayStr");
  const частота = (даты) => {
    t.set("data.pastel.entries", даты.map((d, i) => ({ id: "f" + i, date: d, courseId: "__d", createdAt: i })));
    return t.get("paceOftenText")(t.get("entries")());
  };
  t.set("todayStr", () => "2026-09-07");
  ок("частота: почти каждый день",
    частота(["2026-09-02","2026-09-03","2026-09-04","2026-09-05","2026-09-06","2026-09-07"]),
    "почти каждый день");
  ок("частота: через день",
    частота(["2026-09-01","2026-09-03","2026-09-05","2026-09-07"]), "через день");
  ок("частота: пару раз в неделю",
    частота(["2026-08-31","2026-09-04","2026-09-07"]), "пару раз в неделю");
  ок("частота: из одного захода ничего не следует", частота(["2026-09-07"]), "");

  /* Реже раза в неделю — пишем честно, а не «ноль раз». */
  t.set("todayStr", () => "2026-10-01");
  ок("рисунок: давно не садился — так и говорим",
    /не рисую/.test(t.get("paceHTML")()), true);
