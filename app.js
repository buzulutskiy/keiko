...
/* Сколько минут дня остаётся к началу этого захода. Минуты пишутся в запись
   дня живьём, прямо во время занятия (pracCount дописывает прирост), поэтому
   «сыграно сегодня» уже включает текущий заход — вычитаем его обратно.
   Знаменатель считается один раз, на входе в занятие, и по ходу не едет:
   сел — «0 из 30», отыграл пятнадцать и зашёл снова — «0 из 15», добрал день
   до конца — просто «0 мин» без всякого плана. */
function pracLeftToday(counted) {
  const п = pianoWeekPlan();
  if (п.набрано) return 0;
  const доЗахода = Math.max(0, п.сегодня - Math.round(Number(counted) || 0));
  return Math.max(0, п.надо - доЗахода);
}
