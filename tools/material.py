...
def check(key, profile):
    ...
    # ── карточки знаний ──
    # Карточек больше нет: всё, что в них стоило сказать, лежит на карте там,
    # где о это спотыкаешься. Проверяем только те материалы, у которых они
    # остались с прежних времён, — новым их не заводят (MATERIAL.md, §11).
    facts = m.get("facts") or []
    if facts: проверь(True, f"карточек: {len(facts)} (старые, новым не заводим)", "")
