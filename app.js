...
function emptyData() {
  return {
    ...
    pianoWeek: 240,  // мягкий ориентир у пьесы: сколько минут в неделю играть
...
/* Первая карта книги лежит в каталоге без суффикса — так её видят и старые
   версии приложения, не знающие про несколько карт. Значит и просить её надо
   без суффикса: с ним файла нет вовсе, и «Мёртвые души» висели в «карта
   загружается…» до самого таймаута, хотя картинка была на месте. */
const mapFirst = (id) => (((artsOf(id) || {}).maps || [])[0] || {}).key || "";
/* Версия картинки карты: меняется вместе с самой картинкой, и по ней же
   ...
   замену. */
const mapKey = (id, key) => "map-" + id +
  (key && key !== mapFirst(id) ? "-" + key : "")
...
/* Старая карта под точкой: ретромап показывает исторический план, привязанный
   к современным координатам. Задаётся у книги в файле разбора — {code, year},
   где code это номер карты на retromap.ru. Ставится там, где город с тех пор
   не переехал: Петербургу такая карта объясняет всё, Байкалу нечего. */
const retroMap = (b) => {
  const r = (artsOf((b || book()).id) || {}).retro;
  return r && r.code ? r : null;
};
const retroUrl = (r, p) =>
  `https://retromap.ru/m.php#l=${encodeURIComponent(r.code)}&z=17&y=${Number(p.lat).toFixed(5)}&x=${Number(p.lon).toFixed(5)}`;
...
const mapFile = (id, key) => CAT_ART_FILE("map-" + id +
  (key && key !== mapFirst(id) ? "-" + key : ""));
...
function bookStats() {
  ...
  /* Сто процентов должны быть достижимы. У сборника текст начинается с пятой
     ...
     упиралась в 98%. Где начинается текст, знают сами главы — первая из них.
     Через startPage это считать нельзя: то поле значит «докуда уже прочитано»,
     и книга показывала бы процент до первой отметки. */
...
function achCommon() {
  ...
  /* Материал может отказаться от лестницы дней вовсе: noDays в каталоге.
     У книги в один вечер сходится слишком многое — первая отметка, ночь,
     выходной, третий день, — и четыре карточки подряд обесценивают каждую.
     У книги остались только проценты; у пьесы и рисунка лестница на месте:
     там прогресса нет вовсе и мерить больше нечем. */
  if ((catOf(curKey()) || {}).noDays) return [];
...
function pianoWeekPlan() {
  const цель = Number(data.pianoWeek) > 0 ? Number(data.pianoWeek) : 240;
  const сег = todayStr();
  ...
  const поДням = new Array(7).fill(0);
  for (const e of (data.piano.entries || [])) {
    if (e.deleted || e.date < от || e.date > сег) continue;
    поДням[(fromStr(e.date).getDay() + 6) % 7] += Number(e.mins) || 0;
  }
  const сегодня = поДням[деньНедели];
  /* В зачёт недели идут ВСЕ отыгранные минуты, сколько бы их ни было. Остаток
     делится на дни, что ещё остались: не играл — завтрашнее число растёт,
     переиграл — уменьшается. Одно правило в обе стороны.
     Пробовали иначе: считать прошедшему дню не больше его нормы, чтобы заход
     про запас не съедал следующий день. Держалось это два дня и было снято —
     механика, в которой лишний час не убавляет работы, объясняется дольше,
     чем действует. Разбор той попытки, если понадобится снова, —
     в истории правок (версии 533–539).
     Норма дня считается от сыгранного ДО сегодня: иначе она таяла бы прямо во
     время занятия. «Набрано» смотрит на всю неделю вместе с сегодняшним. */
  const доСегодня = поДням.reduce((a, b, i) => (i < деньНедели ? a + b : a), 0);
  const впереди = 7 - деньНедели;                          // считая сегодняшний
  const остаток = Math.max(0, цель - доСегодня);
  const надо = остаток ? Math.ceil(остаток / впереди) : 0;
  const неделя = поДням.reduce((a, b) => a + b, 0);
  const набрано = неделя >= цель;
  return { цель, неделя, сегодня, доСегодня,
           надо: набрано ? 0 : надо, впереди, набрано };
}

/* Сколько минут набрано СЕГОДНЯ и сколько их надо — для строки занятия.
   Раньше числитель считал текущий заход, и каждый новый начинался с нуля:
   отыграл двадцать, вышел, зашёл снова — опять «0 из 15». Выглядело так,
   будто сделанное не считается. Теперь числитель — весь день: «0 из 35»,
   «20 из 35», «50 из 35». С нового дня он сам начинается с нуля, потому что
   считает по отметкам этого дня, а не по заходам.
   Записанные минуты обновляются раз в минуту (`pracFlush`), поэтому к ним
   добавляем ещё не записанный хвост текущего захода — иначе цифра стояла бы
   на месте по минуте и казалась замершей. */
function pracDayMins() {
  const п = pianoWeekPlan();
  const живьём = prac && prac.startedAt ? Math.max(0, pracMin() - (prac.counted || 0)) : 0;
  return { всего: Math.round(п.сегодня + живьём), надо: п.надо, набрано: п.набрано };
}
...
/* Часы на пианино в неделю. Ручки для этого не было вовсе: число лежало в
   профиле и менялось только правкой данных, то есть цель была не целью, а
   константой. Стоит рядом с целью по дням — оба вопроса про неделю, и
   спрашивают их в одном месте.
   Показываем только тем, у кого есть пьеса: без инструмента это пустая
   строка про чужую жизнь. */
function pianoGoalUI() {
  const пьесы = (data.piano.pieces || []).filter((p) => !p.archived);
  if (!пьесы.length) return "";
  const цель = Number(data.pianoWeek) > 0 ? Number(data.pianoWeek) : 240;
  const п = pianoWeekPlan();
  /* В часах, а не в минутах: думают об этом часами («играю четыре часа в
     неделю»), а делится оно потом само. */
  const часы = [2, 2.5, 3, 4, 5, 6];
  const подпись = (ч) => String(ч).replace(".5", "½");
  return `
    <div class="freeze">
      <div class="fz-head">🎹 <b>Часы на пианино</b> — сколько играть в неделю</div>
      <div class="goal-pick">
        ${часы.map((ч) => `<button class="gbtn ${Math.round(ч * 60) === цель ? "on" : ""}"
          data-pgoal="${Math.round(ч * 60)}" type="button">${подпись(ч)}</button>`).join("")}
      </div>
      <div class="fz-empty">Сейчас: <b>${п.неделя} из ${цель} мин</b> на этой неделе${
        п.набрано ? "" : ` · сегодня ${п.надо}`}</div>
    </div>`;
}

function bindPianoGoalUI() {
  document.querySelectorAll("[data-pgoal]").forEach((b) =>
    b.addEventListener("click", () => {
      data.pianoWeek = Number(b.dataset.pgoal);
      /* Метка нужна слиянию: цель мержится по времени правки, как weekGoal. */
      data.pianoWeekAt = now();
      saveData(); schedulePush();
      openSettingsSheet();
      render();
      const ч = data.pianoWeek / 60;
      toast(`Пианино: ${String(ч).replace(".5", "½")} ${plural(Math.round(ч), "час", "часа", "часов")} в неделю`);
    }));
}
...
function saveEntry() {
  ...
  /* Итог дочитанной книги — экран на всю книгу, а не на отметку: вечера,
     часы, страницы, заметки. Он остаётся: это не похвала за нажатие кнопки,
     а единственное место, где видно весь заход целиком. Идёт первым, до
     наград: сначала «книга закрыта», потом то, что за неё пришло. */
...
/* Какая глава стоит в подписи у книги подряд и сколько в ней осталось.
   Главное здесь — дочитанная глава: курсор на её последней странице значит, что
   она кончилась, и смотреть надо вперёд. Иначе имя оставалось прежним, а число
   молча переставало быть про него: дочитав первую главу «Мёртвых душ» на
   двадцать второй странице, человек видел «Глава I · осталось 329 страниц» —
   это уже остаток тома. Теперь в таком месте стоит следующая глава и её
   собственная длина. Null — считать нечего: глав нет или книга кончилась. */
function heroChapter(s) {
  const b = book();
  const гл = b.chapters || [];
  if (гл.length < 2) return null;
  const i = гл.findIndex((c) => Number(c.from) === Number((s.chapter || {}).from));
  if (i < 0) return null;
  const конец = (j) => j + 1 < гл.length ? Number(гл[j + 1].from) - 1 : (Number(b.pages) || 0);
  let j = i, курсор = Number(s.page) || 0;
  if (курсор >= конец(i) && i + 1 < гл.length) {
    /* Глава дочитана: переходим к следующей и считаем её целиком. */
    j = i + 1;
    курсор = Number(гл[j].from) - 1;
  }
  /* Неоткрытая книга считается от первой страницы текста, а не от нуля. */
  const откуда = Math.max(курсор, (Number(гл[j].from) || 1) - 1);
  return { name: гл[j].name || "", left: Math.max(0, конец(j) - откуда) };
}
...
function heroParts(s) {
  ...
  if (isBook()) {
    /* Сколько осталось В ЭТОЙ ГЛАВЕ, а не во всей книге: рядом стоит её имя, и
       число обязано говорить про неё. Считать нечего — имя уходит вместе с ним,
       иначе число молча меняет предмет. Правило то же, что у сборника повестей,
       только там куски, а тут главы подряд. */
    const г = heroChapter(s);
    return г && г.left ? [esc(г.name), `осталось ${stranic(г.left)}`]
                       : [`осталось ${stranic(bookLeft(s))}`];
  }
...
function bookBtnState() {
  /* Три состояния, а не два. `on` — показать кнопку сейчас. `keep` — держать
     под неё место, даже когда её нет: у книги разбор приезжает из каталога
     позже, и появившаяся кнопка не должна двигать раскладку на полслова.
     Место держим только там, где кнопка ещё может появиться: у обычной книги.
     У пьесы карты и собрания нет вовсе, у сборника статей — тоже, у курса и
     ролика состояние известно сразу. Там пустой дырки рядом с «Отметить»
     быть не должно — кнопка «Отметить» занимает всю ширину. */
  /* Ждать нечего только у пьесы и у сборника статей: там кнопки нет и не
     будет. У курса она есть (у «Аргуса» собрание), и место под неё держим,
     пока музей едет из гиста, — иначе приехавшая кнопка дёрнет раскладку. */
  /* `noMap` у книги — «карты и собрания здесь не будет никогда». Ставится
     руками при заведении: у собрания сочинений Пушкина размечать нечего —
     восемьсот страниц разных вещей, — и держать пустое место под кнопку,
...
function humanWhen(d, days) {
  ...
  /* Числа тут не было бы вовсе. «Примерно к 17 сентября» звучит как
     обещание, которого прогноз дать не может: он пересчитывается каждый
     вечер и на таком расстоянии гуляет на неделю. Месяц делится на начало,
     середину и конец — этого хватает, чтобы представить, куда это попадает
     в жизни, и нечему сдвигаться на глазах. */
...
function pracWhereText() {
  ...
  const д = pracDayMins();
  /* Превышение показываем отдельным хвостом: «50 из 35» и так читается, но
     «+15» отвечает на вопрос сразу, без вычитания в уме. */
  if (!д.надо) return `${д.всего} мин`;
  const сверх = д.всего - д.надо;
  return `${д.всего} из ${д.надо} мин` + (сверх > 0 ? ` · +${сверх}` : "");
}
...
function openPlaceMap(bk, i, выбрать) {
  ...
  /* Подложки может не быть, хотя места размечены: картинку карты заливают
     отдельно и позже. Это не повод не открываться — вкладка «Места» просто не
     появится, а собрание и текст на месте. Отказываем, только когда показывать
     нечего совсем. */
  const подложка = !!mapBox(bk);
  if (!места.length && !естьСобрание) { toast("Карты пока нет"); return; }
  const box = $("#gmap");
  if (!box) return;
  ...
  gm = { места, рамка, карта: "", карты: mapMaps(bk), at: выбрать || null, часть, слой: "", scale: 1, tx: 0, ty: 0, id: bk.id, i,
         ...
         подложка, мат: bk, вид: isCourseBook(bk.id) ? "pastel" : (data.piano.pieces || []).some((p) => p.id === bk.id) ? "piano" : "book",
...
/* «Справки» и «Слова» с карты сняты: справки целиком переехали в собрание, а
   ...
   Оговорка про «Слова»: с этой правки на экран разбора непонятных слов не
   ведёт ничего. Открывал его только `gmList` при слое "ask", а выбрать этот
   слой стало неоткуда — `askPane`, `askAdd`, `askRun` живы и недостижимы.
...
function gmLayersOf() {
  ...
  /* Без картинки карты ставить точки некуда — вкладку не показываем. Флага
...
/* Сколько мест в главе. Раньше здесь стояла разбивка по всем слоям — «3 места
   · 2 книги · 7 слов», — но книги, люди и слова уехали в собрание, и на этом
   экране их нет. Подпись обещала то, чего по нажатию не покажут. */
function gmМест(по) {
  const n = (по || {}).place || 0;
  return n ? `${n} ${plural(n, "место", "места", "мест")}` : "";
}
const gmВидимые = () => {
  if (!gm) return [];
...
/* Слова не привязаны к главе жёстко и по ней не стираются. Пробовали стирать —
   и это оказалось ловушкой: «текущую главу» двигает только отметка страницы,
   значит чистка срабатывала бы ровно в тот момент, когда человек отметил день,
   и кнопка «Разобрать» на том же экране показывала бы ноль. Плюс короткие
   рассказы читают по два-три за раз, не прерываясь на отметку, — граница главы
   там всё равно врёт. Глава хранится в записи, но нужна только для порядка:
   ближайшее к текущему месту показывается первым. */
const asksOpen = (id) => {
  const кн = id || (book() || {}).id;
  return asksAll().filter((a) => !a.deleted && !a.done && a.bookId === кн)
    .sort((a, b) => Math.abs((Number(b.ch) || 0) - askГлава()) - Math.abs((Number(a.ch) || 0) - askГлава()));
};
...
function askSuggest(начало, сколько) {
  ...
  return askIndex.слова.filter((x) => x.w.startsWith(p))
    .sort((a, b) => a.d - b.d || a.n - b.n || a.w.length - b.w.length)
...
function gmNavRow() {
  ...
  /* Выбор карты показываем, только когда он настоящий: в этой главе точки
     правда лежат на разных картах. У «Мёртвых душ» так вышло у трёх глав из
     одиннадцати и на «всех местах»; в остальных карта одна, и селект был бы
     кнопкой без выбора. Какая карта — решают сами точки, а не человек.
     И в списке только те карты, где у этой главы точки есть: у книги их может
     быть четыре, а в главе заняты две — остальные открывались бы пустыми. */
  const ключи = new Set(gmМестаГлавы().map(картаТочки));
  const свои = карты.filter((m) => ключи.has(m.key || ""));
  const разные = свои.length > 1;
  gmMapRow(свои, своя && разные);
  ряд.hidden = !своя || (!главы && !искать && !разные);
  if (кн) {
    кн.hidden = !главы;
...
function gmLayersRow() {
  ...
  /* Пустая вкладка в этой главе не показывается совсем: «Справки · 0» —
     ...
     «Текст» и «Слова» считать точками бессмысленно — там их нет вовсе, и без
     этой оговорки «Слова» не показывались никогда. */
  /* Вкладки без точек на карте: «Текст», «Слова», «Собрание». Забыть внести
     сюда новую значит потерять её молча — так уже случилось дважды. */
  const свои = new Set(["place", "read", "ask", "col"]);
  const вкладки = gmLayersOf().map(([k, имя]) => [k, имя, счёт(k)])
    .filter(([k, , n]) => n || свои.has(k));
  ...
  box.innerHTML = вкладки.map(([k, имя, n]) =>
    `<button data-layer="${k}"${k === текущий ? ' class="on"' : ""}
      type="button">${имя}${свои.has(k) && k !== "place" ? "" : " · " + n}</button>`).join("");
...
function gmToc() {
  ...
  const вне = gm.места.filter((p) => !частьТочки(p) && слойТочки(p) === "place").length;
  ...
  const вокруг = вне ? `<button data-part="-1"${выбрана(-1)} type="button">${птица(-1)}Вокруг книги
        <em>${слово(вне)} · вне глав</em></button>` : "";
  box.innerHTML = вокруг + части.map((c) => `<button data-part="${esc(String(c.n))}"${выбрана(Number(c.n))} type="button">${птица(Number(c.n))}${esc(gmИмяГлавы(c.n, c.name))}
        <em>${esc(gmМест(c.по))}</em></button>`).join("");
}
...
function gmCard() {
  ...
  /* Карта тех лет, если у книги она задана: тот же адрес, только план 1828
     года вместо нынешнего. Место оказывается в центре экрана. */
  const ретро = retroMap(gm && gm.id ? { id: gm.id } : null);
  ...
  /* Главный вопрос к месту — не «что тут было», а «что от этого осталось и
     ...
     идут от названия к тому, что запоминается, и заканчиваются дорогой.

     Перечень находок бронзового века не запоминается ничем — поэтому яркие
     факты стоят вторым пунктом, сразу после имени, а археология уехала к
     концу. Отдельно спрашиваем про связь с Россией, и не только военную:
     торговля, наука, живопись, родство династий, эмиграция — по такой ниточке
     чужое место становится своим. И отдельно про вещи родом отсюда: если
     что-то из них лежит в Эрмитаже или Пушкинском, до него можно доехать.
  ...
  const вопрос = encodeURIComponent(
    ...
    + `1) откуда взялось название: из какого языка, что означало, — и как это место называется сейчас на карте; `
    + `2) два-три самых ярких факта об этом месте — то, что запоминается и что про него рассказывают, а не перечень найденных черепков; `
    + `3) связь с Россией, любая и не только военная: кто отсюда бывал в России и кто из России бывал здесь, торговля, наука, искусство, дипломатия, родство, эмиграция. Если связи нет, так и скажи одной строкой; `
    + `4) люди, чьи имена с этим местом связаны, и отдельно те из них, кто известен в России; `
    + `5) знаменитые вещи родом отсюда — что это и где они хранятся сейчас; если что-то из них лежит в российских музеях, назови музей; `
    + `6) что здесь было в ту эпоху, о которой книга, и что от неё уцелело на самом месте; `
    + `7) какая рядом природа и какие природные места стоят внимания — горы, бухты, водопады, пещеры, скалы, озёра по именам; `
    + `8) что стоит увидеть сегодня и как туда добираются. `
  ...
  card.innerHTML = `
    ...
    <div class="gm-links">
      ...
      ${ретро ? `<a href="${esc(retroUrl(ретро, p))}" target="_blank" rel="noopener">Карта ${
        esc(String(ретро.year || ""))}</a>` : ""}
...
/* Первая строка: сколько вещей прочитано из скольких и сколько это страниц.
   Считаем настоящие вещи, а не разделы: «0 из 67» у тома, где пятьсот
...
function colItems(b0) {
  ...
  /* Записи без главы — «вокруг книги»: про перевод, про издание, про премию.
     Сюжета они не касаются, поэтому и лежали открытыми с первого дня. Выходило,
     что у неоткрытой книги в собрании уже что-то собрано, а у «Писем Баламута»,
     где глава не проставлена ни у одной записи, — сразу всё. Теперь они тоже
     привязаны к чтению, только не к главам, а к страницам: раскиданы поровну
     от той, с которой человек начал, до последней. */
  /* Мест в собрании нет. У «Одиссеи» их восемьдесят шесть, у «Столпов моря»
  ...
  /* Одно и то же слово размечено в каждой главе, где встречается: «Стек» у
     «Столпов моря» — двадцать один раз, «Лава» — четырнадцать. На карте это
     верно, там всё разложено по главам и счётчик считает по главе. В собрании
     же главы сведены вместе, и выходил столбик одинаковых ячеек. Оставляем
     одну — ту, где написано больше всего, — а главу берём самую раннюю: вещь
     должна приходить тогда, когда встретилась впервые, а не в последний раз.
     Ключ у них и так один, так что отметка «просмотрено» всё равно была общая. */
  const один = new Map();
  for (const x of из) {
    const был = один.get(x.id);
    if (!был) { один.set(x.id, x); continue; }
    const длина = (y) => (y.about || "").length + (y.t || "").length;
    const глубже = длина(x) > длина(был) ? x : был;
    const раньше = [был.ch, x.ch].filter(Boolean).sort((a, c) => a - c)[0] || 0;
    один.set(x.id, Object.assign({}, глубже, { ch: раньше }));
  }
  из.length = 0; из.push(...один.values());
...
/* Страница, на которой приходит запись без главы. Порядок — по id, а не по
   месту в списке: список пересобирается из карты и музея, а id постоянны, и
   от порядка не должно зависеть, что откроется раньше. */
function colПороги(b, список) {
  if (!b || b.kind !== "book") return список;
  const без = список.filter((x) => !Number(x.ch)).sort((a, c) => (a.id < c.id ? -1 : a.id > c.id ? 1 : 0));
  const от = Number(b.startPage) || 0, до = Number(b.pages) || 0;
  без.forEach((x, i) => {
    x.порог = до > от ? от + Math.round((i + 1) * (до - от) / (без.length + 1)) : 0;
  });
  return список;
}
...
/* Сколько вещей открывает одно занятие. Считаем занятия, а не минуты и не дни:
   ...
   рисунка одинаково. По времени порция зависела раньше, и выходил счётчик, за
   которым тянет досидеть до сорока минут, — ровно то, чего здесь быть не
   должно. */
function colПорция(мат) {
  if (!мат) return 0;
  // одна вещь за занятие, и у пьесы, и у рисунка: два захода за день — две вещи
  if (мат.kind === "piece" || мат.kind === "course") return 1;
  return 0;
}
...
/* Всё, что лежит в разделе, называется одним словом — «произведение».
   Пробовали по видам: у главы стоит `kind`, и строка собиралась точной —
   «492 стихотворения · 55 статей · 13 поэм · 11 в прозе · 7 пьес · 6 сказок ·
...
const lsTotal = (p) => (lsItems(p).length || 1);
