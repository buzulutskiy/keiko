   ...
   втрое, так что от неё нет толку. */
{
  ...
  const предел = t.get("askПредел")(разделы, t.get("data").book.books[0]);
  const м = t.get("askCandidates")(разделы, "фальшфейер", предел);
  ок("слова: нашлось только прочитанное", м.length, 2);
  ок("слова: короткая строка добирается соседней",
    м[0].фраза, "Он достал фальшфейер и поджёг его. Дюльфер — спуск по верёвке, простой и быстрый.");
  const метки = t.get("askМетки")(разделы, t.get("data").book.books[0]);
  ок("слова: раздел подписан именем из содержания",
    t.get("askCandidates")(разделы, "фальшфейер", предел, метки)[1].раздел, "Дальше");

  // склонение: набирают начальную форму, в книге стоит другая
  ок("слова: по основе тоже находит",
    t.get("askCandidates")(разделы, "дюльфера", предел).length > 0, true);
  ок("слова: чего нет — того нет",
    t.get("askCandidates")(разделы, "жумар", предел).length, 0);

  /* Подсказки строим по прочитанному, редкое выше частого. */
  t.get("askBuildIndex")("kn", разделы, предел);
