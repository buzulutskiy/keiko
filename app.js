...
function paceGoal(f) {
  /* Порога по числу заходов здесь нет намеренно. Он был — три, — и из-за него
     строка молчала там, где отметок хватало на дату, но не хватало на него:
     заходы, где страница не сдвинулась, в счёт не идут, и трёх могло не
     набраться месяцами. Требовать от подсказки больше уверенности, чем от
     прогноза, который тут же рядом называет число занятий и месяц, — странно.
...
/* Единственное место, где прогноз превращается в срок. Раньше пересчёт жил
   в двух экранах порознь, и они расходились на месяцы — на главной одно,
   в прогрессе другое. Два разных ответа на один вопрос хуже одного
   приблизительного: непонятно, какому верить.
