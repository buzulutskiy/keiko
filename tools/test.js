...
/* ── Минуты недели: растут от простоя и обнуляются в понедельник ── */
{
  ...
  /* Поиграл в понедельник — во вторник просят меньше. Но в зачёт идёт норма
     дня (22), а не все тридцать: восемь лишних минут остатка не трогают. */
  t.get("data").piano.entries=[{ id: "p1", date: "2026-09-14", pieceId: "bwv853", mins: 30 }];
  день("2026-09-15");
  ок("неделя: сыгранное уменьшает остаток", t.get("pianoWeekPlan")().надо, 22);

  /* Его случай: пять дней без пианино, в субботу два часа. Воскресенье просит
     те же 75 минут, что просило бы и без субботнего запаса. */
  t.get("data").piano.entries=[{ id: "p1", date: "2026-09-19", pieceId: "bwv853", mins: 120 }];
  день("2026-09-20");
  ок("неделя: заход про запас не съедает следующий день",
    t.get("pianoWeekPlan")().надо, 75);
  /* А если бы в субботу отыграл ровно норму — ровно столько же. */
  t.get("data").piano.entries=[{ id: "p1", date: "2026-09-19", pieceId: "bwv853", mins: 75 }];
  день("2026-09-20");
  ок("неделя: норму отыграл или перекрыл — назавтра одно и то же",
    t.get("pianoWeekPlan")().надо, 75);
  /* Недоигранное, наоборот, переносится: в этом весь смысл остатка. */
  ...
  /* Но зачёт при этом взял с понедельника только его норму: два счётчика
     живут врозь, и путать их нельзя. */
  ок("неделя: в зачёт с того дня ушла только норма",
    t.get("pianoWeekPlan")().зачтено, 22);
...
/* ── Мягкий ориентир у пьесы: минуты недели ── */
{
  ...
  ок("минуты: но в зачёт следующим дням лишнее не ушло", п2.зачтено, 0);
  ок("минуты: и просить на сегодня нечего", t.get("pracLeftToday")(0), 0);
  ...
  ок("занятие: в начале дня — весь дневной план", t.get("pracLeftToday")(0), Math.ceil(210/впереди));
  t.get("data").piano.entries=[{ id: "p1", date: сег, pieceId: "bwv853", mins: 15 }];
  ок("занятие: после отыгранных пятнадцати остаток меньше",
    t.get("pracLeftToday")(0), Math.max(0, Math.ceil(210/впереди) - 15));
  ок("занятие: минуты текущего захода из остатка не вычитаются дважды",
    t.get("pracLeftToday")(15), Math.ceil(210/впереди));
  t.get("data").piano.entries=[{ id: "p1", date: сег, pieceId: "bwv853", mins: 300 }];
  ок("занятие: день набран — плана в шапке нет", t.get("pracLeftToday")(0), 0);
