...
function paceHTML() {
  ...
  /* «7 дн. из 12»: сколько дней занимался и сколько прошло с первого захода.
     ...
     считаются занятием. */
  const list = entries();
  const часто = paceOften(list);
  return `<span class="pace">${subLine(
    paceDays(),
    часто > 0 && часто < 1 ? IDLE_WORD() : "",
    paceText(w)
  )}</span>`;
}
...
const DAYS_WORD = () => isBook() ? "с книгой"
  : isPiano() ? "с пьесой"
  : isCourse() && plainDraw() ? "с рисунком"
  : "с курсом";
function paceDays() {
  const n = curStats().days || 0;
  return n ? `${n} ${plural(n, "день", "дня", "дней")} ${DAYS_WORD()}` : "";
}
...
function paceDaysHTML(list) {
  ...
  if (!дни) return "";
  const часто = paceOften(list);
  return `<span class="pace">${subLine(дни, часто > 0 && часто < 1 ? IDLE_WORD() : "")}</span>`;
}
