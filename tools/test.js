...
/* ── Слои карты ── */
{
  ...
  ок("слои: перечислены все три", t.get("gmLayersOf")().map((x) => x[0]), ["place", "book", "person"]);
  ...
  ок("слои: главы считаются по своему слою", t.get("gmParts")().length, 0);
  t.get("gm").слой = "place";
  ок("слои: у мест глава на месте", t.get("gmParts")().map((c) => c.k), [1]);
  t.set("gm", было);
}
